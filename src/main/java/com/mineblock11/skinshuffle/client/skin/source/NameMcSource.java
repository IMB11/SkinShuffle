...
public class NameMcSource implements SkinSource {
    ...
    private static final String PAGE_URL = BASE_URL + "/minecraft-skins/trending?page=%d";
    ...
    private class Page {
        ...
        private Document fetchPage() {
            try {
                return Jsoup.connect(String.format(PAGE_URL, index)).get();
            } catch (IOException e) {
        ...
        private Skin fetchSkin(int index) {
            try {
                var skinPage = Jsoup.connect(document.select(detailsPageSelector)
                                .get(index)
                                .attr("abs:href"))
                        .get();
